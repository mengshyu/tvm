...
def test_max_pool2d(size):
    def create_model() -> tvm.IRModule:
        input0 = relay.var("input0", relay.TensorType((1,1,size,size),dtype="float16"))
    ...
    verify(
        mod,
        inputs={
            "input0": np.random.uniform(size=(1, 1, size, size)).astype("float16"),
        },
        decimal=3,
    )
